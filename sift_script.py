...
if __name__ == '__main__':
    origimg = plt.imread('input/lamp_oneplus.jpg')
    origimg = aug(origimg, albu.Resize(270, 135))
    if len(origimg.shape) ==  3:
        img = origimg.mean(axis=-1)
    else:
        img = origimg
    keyPoints,discriptors = SIFT(img, True)


    origimg2 = plt.imread('input/lamp_oppo.jpg')
    origimg2 = aug(origimg2, albu.Resize(180, 135))
    if len(origimg.shape) == 3:
